...
fn main() {
        ...
        sieve_code = sieve_code
            ...
            + &code_block({
                let mut g = DomainGenerator::new(
                    &domain,
                    domain_as_first_folder
                        || GLOBAL_OPTIONS.lock().unwrap().force_domain_as_first_folder,
                );
    ...
    // Prepend the prefix sieve and write to output
    let content = fs::read_to_string(&args.prefix).unwrap_or_default() + &sieve_code;
